...
This study employed a quantitative, explanatory research design grounded in the Structure-Conduct-Performance (SCP) paradigm. The core hypothesis is that an industry's Structure (its firm-size distribution) is a primary determinant of its Performance (economic resilience and vulnerability to shock). This study operationalizes only the Structure--Performance link of the SCP paradigm, treating firm closure rates as the performance outcome and firm-size distribution as the structural input. The Conduct dimension is outside the scope of the present analysis.

The methodology proceeds in two phases:

\begin{description}
    \item[\textbf{Phase 1: Taxonomic Construction.}] An unsupervised machine learning model (K-Means) is utilized to segment the Philippine economy. It created a new taxonomy by clustering industries based on their firm-size distribution rather than their stated output (PSIC codes).
...
The consolidated 3-digit industry dataset underwent list-wise deletion to remove any industry with null values for key variables (Total Employment or Total Establishments). Outliers were identified via Z-score analysis to determine if they are legitimate structural extremities (e.g., N783 - Human Resources \& Job Pooling) or data artifacts.
...
A Z-score analysis was conducted to identify distributional anomalies within the 3-digit industry dataset. Using a standard threshold of $Z > 3$, four industries were isolated as structural outliers (Table \ref{tab:outliers}). These observations exhibit an Average Firm Size nearly 10 to over 19 times the national mean ($60.88$). This effectively breaks the continuity of the distribution.
...
The Middle archetype represents industries that have partially escaped the Survivalist trap, exhibiting higher SME density and intermediate scale. Together with the Transition archetype, these clusters constitute the industries closest to the structural ideal implied by the ``Missing Middle'' framework, distinguishing them from both the Survivalist monoculture and the capital-isolated Oligopoly. The label ``Missing Middle'' therefore describes the structural gap visible in Figure \ref{fig:structural_dualism}, not a single cluster: both the Middle and Transition archetypes represent partial realizations of the dense, scalable SME layer that this gap currently lacks.
...
In stark contrast, the upper and right-hand sections of the map depict the formal, industrialized economy. The Middle archetype (yellow crosses) occupies the center-right, representing a healthy, competitive "missing middle" with firm sizes ranging between 20 and 100 employees. This cluster indicates a robust ecosystem of small-to-medium enterprises. As we move rightward in scale, we encounter the Transition cluster (blue Xs), where industries are likely consolidating into larger entities, and finally, the Oligopoly archetype (green squares) at the far right, where extreme scale and low SME density characterize capital-intensive sectors. These oligopolies represent massive firms with nearly 1,000 employees, likely dominating sectors with high barriers to entry, such as utilities or heavy industry.
...
The regression analysis between structural composition and pandemic-era business closures validates the central hypothesis. Structure predicts Performance. We observe a marginally significant positive correlation ($r = 0.44$, $p = 0.068$, $\alpha = 0.10$, $n = 18$; underpowered, post-hoc power $\approx 0.58$) between the share of ``Survivalist'' industries within a sector and the total volume of establishment closures. This correlation is suggestive at the section level, though limited by $n=18$ and an ecological inference constraint (see \S 4.5), and should be interpreted as an exploratory signal rather than a confirmatory finding. The regression results are summarized in Table \ref{tab:ols_results}.
...
\begin{figure}[ht]
  \centering
  \Description{Bar charts comparing R-squared and AIC values for two models. Model A (PSIC dummies) achieves R-squared of 1.0 and AIC of -792.79 by construction as a saturated model with zero residual degrees of freedom, making both metrics undefined as performance measures. Model B (cluster-based) uses a single interpretable predictor, yielding R-squared of 0.19 on 18 observations without overfitting, and is the operationally preferred model.}
  \includegraphics[width=\linewidth]{fig_model_comparison.png}
  \caption{Model Comparison. \texorpdfstring{$R^2$}{R-squared} and AIC for PSIC-dummy model (A) versus cluster-based predictor model (B). \textit{Note: Model A achieves $R^2 = 1.0$ by construction as a saturated model ($n = k = 18$). Teal border indicates operationally preferred model.}}
...
Third, the resilience validation suffers from an ecological inference problem. Closure data from PSA Table 9 is available only at the 1-digit PSIC Section level, while the taxonomy is constructed at the 3-digit level. The weighted aggregation from 3-digit clusters to 1-digit sections introduces measurement error, and the reported correlation ($r = 0.44$) reflects section-level, not cluster-level, variation. A post-hoc power analysis for the bivariate OLS with $n = 18$ and $r = 0.44$ at $\alpha = 0.10$ yields power $\approx 0.58$, confirming the test is underpowered. This further supports the characterization of the result as suggestive rather than confirmatory. Obtaining 3-digit closure data from the PSA would enable a direct test of the central hypothesis.
...
Fifth, K-Means clustering assumes spherical, equal-variance clusters and is sensitive to outliers. While the four retained outliers (Z $>$ 3) were justified as structurally meaningful, their retention may bias the Oligopoly cluster centroid. The Oligopoly cluster ($n = 4$) produces a centroid that is sensitive to the inclusion or exclusion of any single industry. While the robustness checks confirm the four-archetype structure is preserved under K-Medoids, the specific centroid values for the Oligopoly archetype should be interpreted with caution. The robustness checks in Section 4.4 address this concern directly. The K-Medoids (PAM) analysis confirms that the four archetypes are preserved under an outlier-robust algorithm, the ARI stability test demonstrates reproducibility across 50 random seeds, and the ANOVA validates that cluster means differ significantly ($p < 0.001$) on both firm size and entropy.
